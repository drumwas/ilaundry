...
/**
 * Parameterized query helper for access module
 */
...
const access = async (user) => {
  try {
    ...
    let staff_roll = await queryAsync(
      `SELECT sr.*, r.rollType FROM tbl_staff_roll sr JOIN tbl_roll r ON sr.main_roll_id = r.id WHERE sr.id = ?`,
      [roll]
    );

    let rolldetail = {};
    if (staff_roll.length > 0) {
      let main_roll = await queryAsync(
        `SELECT * FROM tbl_roll WHERE id = ?`,
        [staff_roll[0].main_roll_id]
      );
    ...
    if (loginas == 0) {
      ...
      var topbardata = await queryAsync(
        `SELECT 
        tbl_customer.id,
        tbl_customer.name,
        tbl_customer.number,
      ...
      let staff_roll = await queryAsync(
        `SELECT * FROM tbl_roll WHERE id = ? AND roll_status = 'active'`,
        [topbardata[0].main_roll_id]
      );
      ...
      var topbardata = await queryAsync(
        `SELECT tbl_admin.id, tbl_admin.name, tbl_admin.number, tbl_admin.email,
         tbl_admin.username, tbl_admin.store_ID, tbl_admin.roll_id,
         tbl_admin.approved, tbl_admin.delet_flage, tbl_admin.img, tbl_admin.is_staff
         FROM tbl_admin WHERE tbl_admin.id = ?`,
        [id]
      );
      ...
      if (rolldetail.rollType === "master") {
        const multiy = await queryAsync("SELECT type FROM tbl_master_shop");
        if (multiy[0].type == 1) {
          var logas = "master";
          var mutibranch = true;
        } else {
    ...
    const masterstore = await queryAsync(
    ...
    if (mutibranch === true && logas == 'master') {
      notification_data = await queryAsync(
        `SELECT * FROM tbl_notification ORDER BY id DESC LIMIT 5`
      );
    } else if ((mutibranch === false && logas == 'master') || (logas == 'store')) {
      notification_data = await queryAsync(
        `SELECT * FROM tbl_notification WHERE received = ? ORDER BY id DESC LIMIT 5`,
        [topbardata[0].store_ID]
      );
    } else {
      notification_data = await queryAsync(
        `SELECT * FROM tbl_notification WHERE received = ? ORDER BY id DESC LIMIT 5`,
    ...
    console.error("Access middleware error:", error.message);
    console.error("Full stack:", error.stack);
    // Return a minimal object to prevent template crashes
    return {
      logas: "master",
      mutibranch: false,
      roll: {},
      topbardata: { name: "Admin", store_ID: "" },
      isstore: false,
      symbol: "$",
      plac: 0,
