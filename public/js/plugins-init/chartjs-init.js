...
(function($) {
 ...
 var dzSparkLine = function(){
	...
	if (jQuery('#areaChart_2').length > 0) {
		...
		let orderdata = document.getElementById("indextosales").value;
		let currencySymbol = document.getElementById("currencySymbol").value || ''; // Fallback empty
		...
		if (orderdata) {
			...
			sdata.forEach(a => {
				...
				splmonth.forEach(b => {
					let splfinal = b.split("#");
					tmonth.push(`${year[0]}/${allmonth[parseFloat(splfinal[0]) - 1]}`);
					tnumber.push(parseFloat(splfinal[1]));
				});
			});
		}

		if (tmonth.length > 12) tmonth = tmonth.slice(-12);
		if (tnumber.length > 12) tnumber = tnumber.slice(-12);

		const ctx = document.getElementById("areaChart_2").getContext('2d');

		const gradient = ctx.createLinearGradient(0, 1, 0, 500);
		gradient.addColorStop(0, "rgba(0, 129, 238, 0.3)");
		gradient.addColorStop(1, "rgba(0, 129, 238, 0)");

		function removeTrailingZero(val) {
			const parsed = parseFloat(val).toFixed(2);
			return parsed.replace(/\.00$/, '').replace(/(\.\d)0$/, '$1');
		}

		function formatIndianScale(value) {
			if (value === 0) return  "0" + " " + currencySymbol;
			if (value >= 1000000000000) {return  removeTrailingZero(value / 1000000000000) + "..."  + currencySymbol};
			if (value >= 10000000000) return removeTrailingZero(value / 10000000000) + "..."+ currencySymbol;
			if (value >= 10000000) return removeTrailingZero(value / 10000000) + "..." + currencySymbol;
			// if (value >= 100000) return removeTrailingZero(value / 100000) + "..." + currencySymbol;
			return value.toLocaleString('en-IN') + " " + currencySymbol;
		}
					...
					tooltip: {
						callbacks: {
							label: function (context) {
								return formatIndianScale(context.raw);
							}
						}
					}
				},
					...
					yAxes: [{
						ticks: {
							...
							callback: function (value) {
								return formatIndianScale(value);
							}
						}
					}],
					xAxes: [{
